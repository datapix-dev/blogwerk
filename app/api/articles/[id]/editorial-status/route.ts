...
export async function GET(
  ...
  const article = await db.article.findFirst({
    ...
    select: {
      ...
      generationJobs: {
        where: { type: "EDITORIAL_ENHANCEMENT" },
        orderBy: { createdAt: "desc" },
        take: 1,
        select: { status: true, errorType: true, errorMsg: true, result: true },
      },
    },
  })
  ...
  const editorialChanges =
    latestJob?.status === "COMPLETED" && latestJob.result
      ? (latestJob.result as Record<string, unknown>).editorialChanges ?? null
      : null
